...
import { QUOTER_V2_ADDRESS } from '../../../../constants';
import { Abi, Address } from 'viem';
import { Options as RetryOptions } from 'async-retry';
import * as retry from 'async-retry';
import * as stats from 'stats-lite';
import * as flatMap from 'lodash/flatMap.js';
...
function onChainQuoteProviderFactory({ getQuoteFunctionName, getQuoterAddress, abi, getCallInputs }: FactoryConfig) {
    return function createOnChainQuoteProvider({ onChainProvider, multicallConfigs: multicallConfigsOverride }: ProviderConfig): QuoteProvider {
        const createGetRoutesWithQuotes = (isExactIn = true) => {
            ...
            return async function getRoutesWithQuote(routes: RouteWithoutQuote[], { blockNumber: blockNumberFromConfig, gasModel }: QuoterOptions): Promise<RouteWithQuote[]> {
                if (!routes.length) {
                    return [];
                }

                const chainId: ChainId = routes[0].amount.currency.chainId;
                const multicallConfigs = multicallConfigsOverride?.[chainId] || BATCH_MULTICALL_CONFIGS[chainId] || BATCH_MULTICALL_CONFIGS[ChainId.ZK_SYNC];
                ...
                const {
                    ...
                    async (_bail, attemptNumber) => {
                        ...
                        if (failedQuoteStates.length > 0) {
                            console.log(`On attempt ${attemptNumber}: ${failedQuoteStates.length}/${quoteStates.length} quotes failed. Reasons: ${reasonForFailureStr}`);
...
function partitionQuotes(quoteStates: QuoteBatchState[]): [QuoteBatchSuccess[], QuoteBatchFailed[], QuoteBatchPending[]] {
    const successfulQuoteStates: QuoteBatchSuccess[] = quoteStates.filter<QuoteBatchSuccess>((quoteState): quoteState is QuoteBatchSuccess => quoteState.status === 'success');

    const failedQuoteStates: QuoteBatchFailed[] = quoteStates.filter<QuoteBatchFailed>((quoteState): quoteState is QuoteBatchFailed => quoteState.status === 'failed');

    const pendingQuoteStates: QuoteBatchPending[] = quoteStates.filter<QuoteBatchPending>((quoteState): quoteState is QuoteBatchPending => quoteState.status === 'pending');

    return [successfulQuoteStates, failedQuoteStates, pendingQuoteStates];
}

function validateSuccessRate(allResults: Result<[bigint, bigint[], number[], bigint]>[], haveRetriedForSuccessRate: boolean, quoteMinSuccessRate: number): void | SuccessRateError {
...
function validateBlockNumbers(successfulQuoteStates: QuoteBatchSuccess[], totalCalls: number, gasLimitOverride?: number): BlockConflictError | null {
...
function processQuoteResults(
    ...
    gasModel: GasModel,
    adjustQuoteForGas: (quote: CurrencyAmount<Currency>, gasCostInToken: CurrencyAmount<Currency>) => CurrencyAmount<Currency>,
): RouteWithQuote[] {
...
export const createV3OnChainQuoteProvider = onChainQuoteProviderFactory({
    getQuoterAddress: (chainId) => QUOTER_V2_ADDRESS.address as Address,
    getQuoteFunctionName: (isExactIn) => (isExactIn ? 'quoteExactInput' : 'quoteExactOutput'),
    abi: quoterV2ABI,
    getCallInputs: (route, isExactIn) => [encodeMixedRouteToPath(route, !isExactIn), `0x${route.amount.quotient.toString(16)}`],
});
