...
import { Pool, FeeAmount } from '@real-wagmi/v3-sdk';
...
interface FactoryParams<M extends PoolMeta, P extends WithTvl> {
    ...
    getPoolsFromSubgraph: (params: { addresses: Address[]; getPoolMetaByAddress: (address: Address) => M | null; client: GraphQLClient }) => Promise<(P | null)[]>;
}

function subgraphPoolProviderFactory<M extends PoolMeta, P extends WithTvl>({ id, getPoolMetas, getPoolsFromSubgraph }: FactoryParams<M, P>) {
    return async function subgraphPoolProvider({ provider, pairs }: { provider?: SubgraphProvider; pairs: [Currency, Currency][] }): Promise<P[]> {
        if (!provider) {
...
const getV3PoolMeta = memoize(
    ([currencyA, currencyB, feeAmount]: [Currency, Currency, FeeAmount]) => ({
        address: computePoolAddress({ tokenA: currencyA.wrapped, tokenB: currencyB.wrapped, fee: feeAmount }),
        currencyA,
        currencyB,
        fee: feeAmount,
    }),
...
const getV3PoolMetas = memoize(
    (pair: [Currency, Currency]) => [FeeAmount.LOWEST, FeeAmount.LOW, FeeAmount.MEDIUM, FeeAmount.HIGH].map((fee) => getV3PoolMeta([...pair, fee])),
    ([currencyA, currencyB]) => {
        if (currencyA.wrapped.equals(currencyB.wrapped)) {
...
interface AllPoolsFactoryParams<P extends WithTvl> {
    ...
    getPoolsFromSubgraph: (params: { lastId: string; pageSize: number; client: GraphQLClient; chainId: number }) => Promise<P[]>;

    getPoolId: (p: P) => string;
}

function subgraphAllPoolsQueryFactory<P extends WithTvl>({ getPoolsFromSubgraph, getPoolId }: AllPoolsFactoryParams<P>) {
    return async function getAllPools({ provider, chainId, pageSize = 1000 }: { chainId?: ChainId; provider?: SubgraphProvider; pageSize?: number }): Promise<P[]> {
        if (!provider || !chainId) {
            throw new Error('No valid subgraph data provider');
        }
        const client = provider({ chainId });
...
export const getAllV3PoolsFromSubgraph = subgraphAllPoolsQueryFactory<V3PoolWithTvl>({
    ...
    getPoolsFromSubgraph: async ({ lastId, pageSize, client, chainId }) => {
        const { pools: poolsFromSubgraph } = await client.request<{ pools: V3DetailedPoolSubgraphResult[] }>(queryAllV3Pools, {
            pageSize,
            id: lastId,
        });

        return poolsFromSubgraph.map(({ id, liquidity, sqrtPrice, tick, totalValueLockedUSD, token0, token1, feeTier }) => {
            return {
                type: PoolType.V3 as const,
                fee: Number(feeTier),
